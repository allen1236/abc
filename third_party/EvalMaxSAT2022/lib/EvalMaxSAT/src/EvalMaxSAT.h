...
public:
    ...
    // Optional termination support (only effective with CadicalInterface).
    void connect_terminator( CaDiCaL::Terminator * t ) {
        auto * ci = dynamic_cast<CadicalInterface*>(solver);
        if (ci) ci->connect_terminator(t);
    }
    void disconnect_terminator() {
        auto * ci = dynamic_cast<CadicalInterface*>(solver);
        if (ci) ci->disconnect_terminator();
    }
    bool terminated() const {
        return solver ? solver->terminated() : false;
    }
private:
        ...
        for(unsigned int i=0; i<nbMinimizeThread; i++) {
         ...
         for(;;) {
            ...
            for(;;) {
                ...
                // If the underlying SAT solver was terminated (time/budget), stop early.
                if (solver->terminated() || resultSolve == -1) {

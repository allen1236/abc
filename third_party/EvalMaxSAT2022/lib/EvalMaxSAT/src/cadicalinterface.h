...
class CadicalInterface : public VirtualSAT {
    ...
    unsigned int nVar = 0;
    //int conflictSize;
    int last_result = 0;        // 0=unknown/limit, 10=sat, 20=unsat
    bool terminated_last = false;
...
public:
   ...
   bool solve() override {
        ...
        last_result = result;
        terminated_last = (result == 0);
        return result == 10;
    }

    std::vector<int> getConflict(const std::vector<int> &assumptions) override {
        std::vector<int> conflicts;
        // CaDiCaL only allows 'failed' queries in UNSAT state.
        if (last_result != 20)
            return conflicts;
   ...
   int solveLimited(const std::set<SetLit> &assumption, int confBudget, int except) override {
    ...
    bool solve(const std::vector<int> &assumption) override {
        ...
        // result == 0: terminated/limit => treat as not SAT
        return false;
    }
   ...
   bool solve(const std::set<SetLit> &assumption) override {
    ...
    bool terminated() const override { return terminated_last; }

    void connect_terminator(CaDiCaL::Terminator * t) { solver->connect_terminator(t); }
    void disconnect_terminator() { solver->disconnect_terminator(); }

...
#include "lib/EvalMaxSAT/src/cadicalinterface.h"
using namespace std;
...
public:
   ...
   void set_terminate(void * st, int (*cb)(void *)) {
       term_state = st;
       term_cb = cb;
       if (term_cb) {
           if (!terminator)
               terminator = std::make_shared<GlueTerminator>(this);
       } else {
           terminator.reset();
       }
   }
   ...
   int32_t solve() {
       ...
       connect_terminator_to(maxSATwithAssump.get());
       bool return_val = maxSATwithAssump->solve();
       disconnect_terminator_from(maxSATwithAssump.get());
...
private:
   struct GlueTerminator : public CaDiCaL::Terminator {
       IpamirSolver * owner;
       explicit GlueTerminator(IpamirSolver * o) : owner(o) {}
       bool terminate() override {
           auto cb = owner->term_cb;
           if (!cb) return false;
           int r = cb(owner->term_state);
           if (r) owner->terminated_flag.store(true, std::memory_order_relaxed);
           return r != 0;
       }
   };
   ...
   std::shared_ptr<GlueTerminator> terminator;
   std::atomic<bool> terminated_flag{false};

   inline bool was_terminated() const {
       return terminated_flag.load(std::memory_order_relaxed);
   }

   inline void connect_terminator_to(EvalMaxSAT * m) {
       terminated_flag.store(false, std::memory_order_relaxed);
       if (!terminator) return;
       m->connect_terminator(terminator.get());
   }

   inline void disconnect_terminator_from(EvalMaxSAT * m) {
       if (!terminator) return;
       m->disconnect_terminator();
   }
...
void ipamir_set_terminate (void * s, void * state, int (*terminate)(void * state)) { import(s)->set_terminate(state, terminate); }
